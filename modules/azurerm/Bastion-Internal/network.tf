...
resource "azurerm_subnet" "bastion_subnet" {
  name                                      = join("-", [var.subnet_name_abbreviation, var.subnet_name])
...
resource "azurerm_route_table" "bastion_route_table" {
  name                = join("-", [var.route_table_name_abbreviation, var.route_table_name])
...
resource "azurerm_network_interface" "bastion_nic" {
  ...
  ip_configuration {
    name                          = join("-", [var.ip_configuration_name_abbreviation, var.ip_configuration_name])
    subnet_id                     = azurerm_subnet.bastion_subnet.id
...
resource "azurerm_application_security_group" "bastion_application_security_group" {
  name                = join("-", [var.application_security_group_name_abbreviation, var.application_security_group_name])
  location            = var.location
  resource_group_name = var.resource_group_name
  tags                = var.tags
}
