...
resource "azurerm_subnet" "bastion_subnet" {
  name                                      = join("-", ["snet-bastion", var.bastion_vm_subnet_name])
...
resource "azurerm_route_table" "bastion_route_table" {
  name                = join("-", ["route", var.bastion_vm_route_table_name])
...
resource "azurerm_network_security_group" "bastion_nsg" {
  name                = join("-", ["nsg-bastion", var.bastion_vm_network_security_group_name])
  location            = var.location
  resource_group_name = var.resource_group_name
  tags                = var.tags
}
...
resource "azurerm_network_interface" "bastion_nic" {
  name                = join("-", ["nic-bastion", var.bastion_vm_nic_name])
  location            = var.location
  resource_group_name = var.resource_group_name
  tags                = var.tags
  depends_on = [
  ...
  ip_configuration {
    name                          = join("-", ["nic-bastion", var.bastion_vm_ip_configuration_name])
    subnet_id                     = azurerm_subnet.bastion_subnet.id
...
resource "azurerm_application_security_group" "bastion_application_security_group" {
  name                = join("-", ["asg-bastion", var.bastion_vm_application_security_group_name])
  location            = var.location
  resource_group_name = var.resource_group_name
  tags                = var.tags
}
